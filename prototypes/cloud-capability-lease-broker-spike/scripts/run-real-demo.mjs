#!/usr/bin/env node
/* eslint-disable complexity, curly, func-style, no-plusplus, no-promise-executor-return, prefer-destructuring, promise/avoid-new, promise/param-names, sort-keys, unicorn/no-useless-undefined, unicorn/prefer-string-replace-all */
...
async function main() {
  ...
  const files = await fetchPullRequestFiles(record.receipt.prUrl, pr.head?.sha);
...
async function fetchPullRequestFiles(prUrl, headSha) {
  const { owner, repo, number } = parsePrUrl(prUrl);
  const files = [];
  for (let page = 1; ; page++) {
    const response = await fetch(
...
async function fetchRawPullRequestFile(input) {
  const response = await fetch(
    `https://raw.githubusercontent.com/${input.owner}/${input.repo}/${input.sha}/${encodeURIComponentPath(input.path)}`,
    { headers: { "user-agent": "capability-broker-runner" } }
  );
  if (!response.ok) {
    throw new Error(`GitHub raw file readback failed: ${response.status}`);
  }
...
function scanForLeaks(value) {
  ...
  const checks = [
    ...
    { id: "bearer-token", match: /Bearer\s+[A-Za-z0-9._-]{20,}/u.test(text) },
    {
      ...
      match: /https:\/\/(?:x|oauth2|token):[^@\s]{16,}@/u.test(text),
    },
...
async function collectProposedFiles() {
  ...
  for (const repoPath of captureReceiptPaths()) {
    files.push({
      content: await readFile(resolve(repoRoot, repoPath), "utf-8"),
      path: repoPath,
    });
  }
