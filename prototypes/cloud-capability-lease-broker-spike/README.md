...
- leak scan: passed
- PR branch checks: `pnpm check`, `pnpm typecheck`, `pnpm typecheck:tsc`
- CodeRabbit: doctor passed, `coderabbit review --agent` returned `findings: 0`

Macroscope is intentionally excluded from the default review loop after expensive invoice spikes. Use CodeRabbit plus targeted local review unless explicitly requested.
