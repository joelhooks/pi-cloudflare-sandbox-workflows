...
async function preparePayloadInSandbox(
  ...
  const result = await runCommand(sandbox, command, {
    ...
    PAYLOAD_JSON_B64: base64Json(payload),
    REQUEST_JSON_B64: base64Json(requestPreview),
...
function buildPayloadSandboxCommand(input: {
  payload: ResolvedFilesPayload;
  requestPreview: CapabilityLeaseRequest;
}): string {
  void input;
...
const fs = require('fs');
const payload = JSON.parse(Buffer.from(process.env.PAYLOAD_JSON_B64, 'base64').toString('utf8'));
const request = JSON.parse(Buffer.from(process.env.REQUEST_JSON_B64, 'base64').toString('utf8'));
fs.mkdirSync('run', { recursive: true });
fs.writeFileSync('run/payload.json', JSON.stringify(payload, null, 2) + '\n');
fs.writeFileSync('run/capability-request.json', JSON.stringify(request, null, 2) + '\n');
fs.writeFileSync('run/sandbox-receipt.json', JSON.stringify({
  generatedAt: new Date().toISOString(),
  note: 'Sandbox generated payload and request metadata only. GitHub capability execution happens in Worker/DO.',
  payloadHash: payload.payloadHash,
  filesRef: payload.filesRef,
  runId: request.runId
}, null, 2) + '\n');
CAPABILITY_PAYLOAD_JS
git add run/payload.json run/capability-request.json run/sandbox-receipt.json
git -c user.name='shitratgit[bot]' -c user.email='shitratgit[bot]@users.noreply.github.com' commit -m 'pin payload-bound capability request'
...
function toAuthenticatedRemote(
  ...
  return {
    remote: `https://x:${encodeURIComponent(tokenSecret)}@${remote.slice("https://".length)}`,
    tokenSecret,
  };
}
