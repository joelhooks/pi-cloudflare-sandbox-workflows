  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "POST" && url.pathname === "/api/capability-runs") {
      ...
      try {
        ...
        return json({ error: "invalid JSON body", ok: false }, { status: 400 });
      }
      const parsed = StartRunRequestSchema.parse(rawBody);
      const workItemId =
        parsed.workItemId ??
        `cloud-capability-lease-${crypto.randomUUID().slice(0, 16)}`;
      const workerUrl = new URL(request.url).origin;
      const stub = capabilityRunStub(env, workItemId);
      return stub.fetch(
        new Request("https://capability-run.local/start", {
          body: JSON.stringify({ ...parsed, workerUrl, workItemId }),
          method: "POST",
        })
      );
    }
    ...
    if (request.method === "GET" && runMatch?.[1]) {
      ...
      return capabilityRunLookupStub(
        env,
        decodeURIComponent(runMatch[1])
      ).fetch(new Request("https://capability-run.local/record"));
    }
    ...
    if (request.method === "GET" && observerMatch?.[1]) {
      return capabilityRunLookupStub(
        env,
        decodeURIComponent(observerMatch[1])
      ).fetch(new Request("https://capability-run.local/observer"));
    }
...
export class CapabilityRun extends DurableObject<Env> {
  ...
  private async start(request: Request): Promise<Response> {
    const body = StartRunRequestSchema.required({
      workerUrl: true,
      workItemId: true,
    ...
    return json({
      ok: true,
      observerUrl: `${body.workerUrl}/observer/${encodeURIComponent(body.workItemId)}`,
      pollId: body.workItemId,
      record: sanitizeRecord(record),
      runId,
      statusUrl: `${body.workerUrl}/api/capability-runs/${encodeURIComponent(body.workItemId)}`,
...
function capabilityRunLookupStub(
  env: Env,
  lookupId: string
): DurableObjectStub<CapabilityRun> {
  return capabilityRunStub(env, lookupWorkItemId(lookupId));
}
...
function lookupWorkItemId(lookupId: string): string {
  const runMatch = lookupId.match(/^run-(.+)-[a-f0-9]{8}$/u);
  return runMatch?.[1] ?? lookupId;
}

function buildSandboxId(runId: string): string {
  const suffix = stableHexSuffix(runId);
  const slug = slugify(runId).slice(0, 46);
  const id = `cap-${slug}-${suffix}`;
...
function stableHexSuffix(value: string): string {
  ...
  for (let index = 0; index < value.length; index += 1) {
    hash = (hash * 31 + value.charCodeAt(index)) % 0xff_ff_ff_ff;
  }
  return Math.trunc(hash).toString(16).padStart(8, "0").slice(-8);
}
...
function isAuthorized(request: Request, env: Env): boolean {
  ...
  return constantTimeEqual(header, `Bearer ${env.ACCESS_TOKEN}`);
}
