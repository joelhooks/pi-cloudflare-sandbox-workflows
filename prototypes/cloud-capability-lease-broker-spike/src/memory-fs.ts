...
export class MemoryFS {
  ...
  async mkdir(
    ...
    if (existing) {
      ...
      throw fsError("EEXIST", `EEXIST: ${path}`);
    }
  ...
  async readFile(
    ...
    const entry = this.requireEntryFollowingSymlinks(path);
    if (entry.kind === "dir") {
  ...
  async rmdir(path: string): Promise<void> {
    const target = this.normalize(path);
    if (target === "/") {
      throw fsError("EBUSY", `EBUSY: ${path}`);
    }
  ...
  async stat(path: string): Promise<MemoryStats> {
    return new MemoryStats(this.requireEntryFollowingSymlinks(path));
  }
  ...
  async symlink(target: string, path: string): Promise<void> {
    ...
    const parent = this.requireDir(this.parent(normalized));
    this.entries.set(normalized, { kind: "symlink", mtimeMs: 0, target });
    parent.children.add(this.basename(normalized));
  }
  ...
  async writeFile(
    ...
    const parent = this.requireDir(this.parent(target));
    const existing = this.entries.get(target);
    if (existing?.kind === "dir") {
      throw fsError("EISDIR", `EISDIR: ${path}`);
    }
  ...
  private requireEntryFollowingSymlinks(
    path: string,
    depth = 0
  ): Exclude<Entry, { kind: "symlink" }> {
    if (depth > 16) {
      throw fsError("ELOOP", `ELOOP: ${path}`);
    }
    const normalized = this.normalize(path);
    ...
    return this.requireEntryFollowingSymlinks(
      this.resolveSymlinkTarget(normalized, entry.target),
      depth + 1
    );
  }
  ...
  private resolveSymlinkTarget(linkPath: string, target: string): string {
    ...
    return this.normalize(`${this.parent(linkPath)}/${target}`);
  }
}
