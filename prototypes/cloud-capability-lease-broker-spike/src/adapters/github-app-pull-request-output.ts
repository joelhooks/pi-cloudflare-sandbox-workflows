...
async function upsertBranch(input: {
  ...
  const createResponse = await fetch(
    `${githubApiBase}/repos/${input.owner}/${input.repo}/git/refs`,
    {
      body: JSON.stringify({
...
async function upsertPullRequest(input: {
  ...
  const existing = await githubJson<GitHubPullResponse[]>(
    `/repos/${input.owner}/${input.repo}/pulls?head=${input.owner}:${input.branch}&base=main&state=open`,
    input.auth
  );
  if (existing.length > 0) {
    ...
    return githubJson<GitHubPullResponse>(
      `/repos/${input.owner}/${input.repo}/pulls/${pull.number}`,
      input.auth,
